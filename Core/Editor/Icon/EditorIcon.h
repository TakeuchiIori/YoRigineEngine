#pragma once
//=================================================================
// FontAwesome 5 Free — アイコン Unicode 定義 (Modern C++ 版)
//=================================================================
namespace Icon {
    static constexpr const char* Play = "\uf04b";
    static constexpr const char* Xmark = "\uf00d";
    static constexpr const char* Check = "\uf00c";
    static constexpr const char* Danger = "\uf071";
    static constexpr const char* Bolt = "\uf0e7";
    static constexpr const char* Bullhorn = "\uf0a1";
    static constexpr const char* Refresh = "\uf021";
    static constexpr const char* Stop = "\uf04d";
    static constexpr const char* PlusCircle = "\uf055";
    static constexpr const char* Trash = "\uf1f8";
    static constexpr const char* Cog = "\uf013";
    static constexpr const char* FloppyDisk = "\uf0c7";
    static constexpr const char* FolderOpen = "\uf115";
    static constexpr const char* CheckCircle = "\uf058";
    static constexpr const char* TimesCircle = "\uf057";
}