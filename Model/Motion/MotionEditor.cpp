...
void MotionEditor::DrawTimeline()
{
    ...
    float canvasX = ImGui::GetCursorScreenPos().x;
    float canvasY = ImGui::GetCursorScreenPos().y;
    float canvasW = ImGui::GetContentRegionAvail().x;
    float labelW = kLabelW;
    ImDrawList* dl = ImGui::GetWindowDrawList();
    ...
    ImGui::DragFloat("##zoom", &timelineZoom_, 1.0f, 10.0f, 600.0f, "%.0fpx");
    if (ImGui::IsItemHovered()) ImGui::SetTooltip("タイムラインの拡大率 (px/秒)\nマウスホイール + Ctrl でも変更できます");

    ImGui::SameLine(0, 8);
    if (ImGui::SmallButton("|<")) { timelineScroll_ = 0; scrubTime_ = 0; }
    ...
    // ---- キャンバス開始位置を更新 ----
    canvasY = ImGui::GetCursorScreenPos().y;

    // ---- ルーラー (時間目盛り) ----
    float rulerH = 20.0f;
    {
        ImVec2 rMin(canvasX + labelW, canvasY);
        ImVec2 rMax(canvasX + canvasW, canvasY + rulerH);
        dl->AddRectFilled(rMin, rMax, Col::RulerBg);

        // 目盛り間隔を自動計算: おおよそ 60px ごとに 1 目盛り
        float secPerMark = std::max(1.0f, std::ceil(60.0f / timelineZoom_));
        for (float t = 0.0f; t <= duration + secPerMark; t += secPerMark) {
            float x = TimeToX(t, canvasX + labelW);
            if (x < canvasX + labelW || x > canvasX + canvasW) continue;
            dl->AddLine(ImVec2(x, canvasY), ImVec2(x, canvasY + rulerH), Col::RulerLine);
            char buf[16];
            snprintf(buf, sizeof(buf), "%.1fs", t);
            dl->AddText(ImVec2(x + 2, canvasY + 4), Col::RulerText, buf);
        }
        // ルーラーをクリックしてスクラブ
        if (ImGui::IsMouseDown(0)) {
            ImVec2 mp = ImGui::GetMousePos();
            if (mp.x > rMin.x && mp.x < rMax.x && mp.y > rMin.y && mp.y < rMax.y) {
                scrubTime_ = std::clamp(XToTime(mp.x, canvasX + labelW), 0.0f, duration);
            }
        }
    }
    ImGui::Dummy(ImVec2(canvasW, rulerH));

    // ---- ボーン行 (T / R / S 各 1 行) ----
    float rowY = ImGui::GetCursorScreenPos().y;
    int rowIdx = 0;
    for (auto& [boneName, nodeAnim] : nodeAnims) {
        // ラベル背景
        bool isBoneSel = (selKF_.boneName == boneName || selBone_ == boneName);
        ImU32 lbBg = isBoneSel ? IM_COL32(55, 75, 120, 255) : Col::LabelBg;

        // 3 チャンネル × kRowH の高さ
        float groupH = kRowH * 3;
        dl->AddRectFilled(ImVec2(canvasX, rowY), ImVec2(canvasX + labelW, rowY + groupH), lbBg);
        // ボーン名 (中央揃え)
        ImVec2 textPos(canvasX + 4, rowY + groupH * 0.5f - 6);
        dl->AddText(textPos, isBoneSel ? Col::LabelSel : Col::LabelText, boneName.c_str());
        // 区切り線
        dl->AddLine(ImVec2(canvasX + labelW, rowY), ImVec2(canvasX + labelW, rowY + groupH),
            IM_COL32(70, 70, 70, 255));

        // T / R / S 各チャンネル行
        DrawKFRow(dl, boneName, KFChannel::Translate, rowY + kRowH * 0, canvasX, canvasW, labelW);
        DrawKFRow(dl, boneName, KFChannel::Rotate, rowY + kRowH * 1, canvasX, canvasW, labelW);
        DrawKFRow(dl, boneName, KFChannel::Scale, rowY + kRowH * 2, canvasX, canvasW, labelW);

        // 行下の区切り線
        dl->AddLine(ImVec2(canvasX, rowY + groupH), ImVec2(canvasX + canvasW, rowY + groupH),
            IM_COL32(30, 30, 30, 200));

        // ImGui スペース確保
        ImGui::Dummy(ImVec2(canvasW, groupH));
        rowY += groupH;
        ++rowIdx;
    }

    // ---- 再生ヘッド ----
    float headX = TimeToX(scrubTime_, canvasX + labelW);
    if (headX >= canvasX + labelW && headX <= canvasX + canvasW) {
        // 縦線
        dl->AddLine(ImVec2(headX, canvasY), ImVec2(headX, rowY),
            Col::Playhead, 1.5f);
        // 上部三角マーカー
        dl->AddTriangleFilled(
            ImVec2(headX - 5, canvasY),
            ImVec2(headX + 5, canvasY),
            ImVec2(headX, canvasY + 8),
            Col::Playhead);
    }
    ...
    if (draggingKF_ && !ImGui::IsMouseDown(0)) {
        draggingKF_ = false;
        // ドラッグ終了時: 現在の時刻で確定 (MoveKeyframe 内でコマンド登録済み)
        statusMsg_ = "KF 移動完了";
    }
#endif
}
    ...
    float rowY, float canvasX, float canvasW, float labelW)
{
    ...
    // チャンネルに対応するキーフレームリストを取得
    int kfCount = 0;
    auto getTime = [&](int i) -> float {
        switch (ch) {
        case KFChannel::Translate: return na.translate.keyframes[i].time;
        case KFChannel::Rotate:    return na.rotate.keyframes[i].time;
        case KFChannel::Scale:     return na.scale.keyframes[i].time;
        }
        return 0;
        };
    switch (ch) {
    case KFChannel::Translate: kfCount = (int)na.translate.keyframes.size(); break;
    case KFChannel::Rotate:    kfCount = (int)na.rotate.keyframes.size(); break;
    case KFChannel::Scale:     kfCount = (int)na.scale.keyframes.size(); break;
    }

    // 行背景
    float areaX = canvasX + labelW;
    bool isSel = (selKF_.boneName == boneName && selKF_.channel == ch);
    ImU32 rowBg = isSel ? IM_COL32(50, 65, 110, 180) : IM_COL32(40, 40, 40, 160);
    dl->AddRectFilled(ImVec2(areaX, rowY), ImVec2(canvasX + canvasW, rowY + kRowH), rowBg);

    // チャンネルラベル (行左端の小文字)
    const char* chLabel = (ch == KFChannel::Translate) ? "T" :
        (ch == KFChannel::Rotate) ? "R" : "S";
    ImU32 chCol = ChannelColor(ch, false);
    dl->AddText(ImVec2(areaX + 2, rowY + 3), chCol, chLabel);

    // KF ひし形
    for (int ki = 0; ki < kfCount; ++ki) {
        float t = getTime(ki);
        float kx = TimeToX(t, areaX);
        if (kx < areaX || kx > canvasX + canvasW) continue;

        float ky = rowY + kRowH * 0.5f;
        bool  kfSel = (selKF_.boneName == boneName && selKF_.index == ki && selKF_.channel == ch);
        ImU32 col = ChannelColor(ch, kfSel);
        float r = kfSel ? 6.0f : 5.0f;
        ...
        // アウトライン
        dl->AddQuad(ImVec2(kx, ky - r), ImVec2(kx + r, ky), ImVec2(kx, ky + r), ImVec2(kx - r, ky),
            IM_COL32(20, 20, 20, 200), 1.0f);

        // ---- クリック: 選択 ----
        if (ImGui::IsMouseClicked(0) && !draggingKF_) {
            ImVec2 mp = ImGui::GetMousePos();
            float dx = mp.x - kx, dy = mp.y - ky;
            if (std::abs(dx) + std::abs(dy) < r + 4) {
                selKF_.boneName = boneName;
                selKF_.index = ki;
                selKF_.channel = ch;
                scrubTime_ = t;
                selBone_ = boneName;
                SyncJointToBuffer(boneName);
                statusMsg_ = boneName + " [" + chLabel + "] KF " + std::to_string(ki) + " を選択";
            }
        }
        ...
        if (kfSel) {
            if (ImGui::IsMouseClicked(0)) {
                ...
                if (std::abs(dx) + std::abs(dy) < r + 4) {
                    draggingKF_ = true;
                    dragKFOrigTime_ = t;
                    dragKFStartMX_ = ImGui::GetMousePos().x;
                }
            }
            if (draggingKF_ && ImGui::IsMouseDown(0)) {
                float delta = ImGui::GetMousePos().x - dragKFStartMX_;
                float newT = std::clamp(dragKFOrigTime_ + delta / timelineZoom_,
                    0.0f, currentMotion_->GetDuration());
                // リアルタイムに位置更新（Undo はドラッグ完了後）
                MoveKeyframe(boneName, ch, ki, newT);
                scrubTime_ = newT;
                dragKFOrigTime_ = newT;
                dragKFStartMX_ = ImGui::GetMousePos().x;
            }
        }
    }
    ...
    if (ImGui::IsMouseDoubleClicked(0)) {
        ImVec2 mp = ImGui::GetMousePos();
        if (mp.x > areaX && mp.x < canvasX + canvasW &&
            mp.y > rowY && mp.y < rowY + kRowH) {
            selBone_ = boneName;
            SyncJointToBuffer(boneName);
            statusMsg_ = "ボーン選択: " + boneName;
        }
    }
}
#endif
